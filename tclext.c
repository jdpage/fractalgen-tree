...
int frameCmd(CMD_ARGS) {
	...
	if (objc != 6) {
		Tcl_WrongNumArgs(interp, 1, objv, "size branch level seed image");
		return TCL_ERROR;
	}
	
	if (Tcl_GetIntFromObj(interp, objv[1], &size) == TCL_ERROR)
		return TCL_ERROR;
	if (Tcl_GetIntFromObj(interp, objv[2], &branch) == TCL_ERROR)
		return TCL_ERROR;
	if (Tcl_GetIntFromObj(interp, objv[3], &level) == TCL_ERROR)
		return TCL_ERROR;
	if (Tcl_GetIntFromObj(interp, objv[4], &seed) == TCL_ERROR)
		return TCL_ERROR;
	himage = Tk_FindPhoto(interp, Tcl_GetString(objv[5]));
	...
	for (k = 0; k < 4; k++) frame.offset[k] = 0;
